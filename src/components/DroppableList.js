...
const DroppableList = ({scope}) => {
  ...
  return (
    <div ref={drop} className={`min-w-80 shadow-3xl rounded-sm bg-lime-950 text-white flex mx-2 my-1 px-4 py-2  ${isDropping}?'bg-green-800':'bg-white-800'`}>
      <h4 className='text-md font-thin text-align-center'>{scope} scope</h4>
      
      <div className='flex flex-wrap flex-row '>
        {droppedItems.map((item) => (
          <p className=' shadow-sm bg-red-800 text-white m-2 p-2 text-center rounded-sm ' key={item.id}>{item.content}</p>
        ))}
      </div>
